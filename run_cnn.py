...
from model.Model import fact_model,sum_model_bert
...
class Beam_fact(object):
  ...
  def extend(self, token, log_prob, state, context, coverage, graph = None, coverage2 = None, flow = None, step=None):
     ...
     if step != 0 and token !=100:
         ...
         self.add_score=torch.log(fact_score)*0.01
     else:
         self.add_score=0
...
class Train(object):
    ...
    def __init__(self, config):
        ...
        if config.mid_start == 0:
            ...
            x=torch.load('save_model_no_limit/'+config.mid_model,map_location='cpu')
            self.model=x['model']
            self.model.set_cuda()
        ...
        #self.scheduler = NoamLR(self.optimizer, warmup_steps=3000*2)        
    ...
    def save_model(self, running_avg_loss,loss_list,rouge1,rouge2):
        ...
        model_save_path = 'save_model_no_limit/'+str(self.iter)+'_iter_of_'+str(self.ecop) +'_ecop__rouge_'+str(rouge1)+'_'+str(rouge2)+'__loss_'+str(running_avg_loss)
    ...
    def train_one_batch(self):       
        ...
        if self.config.graph_only == 0 and self.config.doc_only == 0:
            ...
            for di in range(min(max_dec_len, self.config.max_dec_steps)):
                ...
                step_loss = step_loss + loss_bs_mean
...
#                sys.stdout = savedStdout  #恢复标准输出流                  
            ...
            if count % self.config.savefreq == 0 and count > self.config.savefreq_min:     
                len_loss=len(loss_list)
                if len_loss <=500*int(32/self.config.mini_batch):
                    recent_loss=loss_list[0:]
                else:
...
class Test(object):
    ...
    def __init__(self, config):
        ...
        x=torch.load('save_model_no_limit/'+config.test_model,map_location='cpu')
...
def argLoader():
    ...
    parser.add_argument('--train_path', type=str, default='data_file/cnn/train/*')

    parser.add_argument('--val_path', type=str, default='data_file/cnn/val/*')

    parser.add_argument('--test_path', type=str, default='data_file/cnn/test/*')
    ...
    parser.add_argument('--savefreq', type=int, default=3000)
    
    parser.add_argument('--savefreq_min', type=int, default=20000)
    ...
    parser.add_argument('--beam_with_fact', type=int, default=0) 
